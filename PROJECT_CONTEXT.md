...
当前最新版本号是 `v=10`，缓存名是 `sedona-workbook-v10`。

## 2026-06-26 UI 工作记录：九大情绪选择器

本次把引导页中的九大情绪选择器重做为更美观的图形化卡片：

- 保留原书九大情绪顺序：万念俱灰、悲苦、恐惧、贪求、愤怒、自尊自傲、无畏、接纳、平和。
- 颜色采用反向彩虹逻辑：左侧低能量为紫蓝冷色，向右逐步过渡到橙红，最右侧“平和”为红色系。
- 宽屏/较宽移动端默认两行布局：第一行 5 个、第二行 4 个；小屏下自动变为 3 列以保证触控面积。
- 情绪卡片默认只显示主情绪、图标和简短 tone，不再直接露出子情绪，减少页面噪音。
- 点击任意主情绪卡片会把上方 `feeling` 输入框填入该主情绪，并标记卡片选中。
- 长按主情绪卡片会展开一个浮层，展示该主情绪下面的子情绪 chip。
- 点击浮层里的任意子情绪也只会填入对应主情绪，不填入子情绪文本，符合“先选主情绪分类”的练习逻辑。
- 同一时间只保留一个情绪卡片展开，避免多个子情绪浮层堆叠。
- 已同步更新缓存版本：`index.html` 的 CSS/JS query 参数为 `v=9`，`service-worker.js` 的 `CACHE_NAME` 为 `sedona-workbook-v9`。

## 2026-06-26 UI 工作记录：情绪选择器二次打磨

根据实际手机截图，v9 存在“情绪卡片被塞进深绿色提问卡片、emoji 图标风格不可控、视觉过重”的问题。本次 v10 做了二次打磨：

- 调整结构：第二步中深绿色 `prompt-card` 只保留提问、输入框和“进入引导”按钮；九大情绪选择器被移出深绿色卡片，作为下方浅色纸面区域的辅助选择模块。
- 调整交互兼容：情绪选择器不在 form 内部后，点击主情绪/子情绪仍会正确填入当前 release setup form 的 `feeling` 输入框。
- 去掉系统 emoji 图标，改用统一的中文单字徽章：灰、悲、惧、求、怒、傲、勇、纳、和，避免不同设备渲染成花哨贴纸。
- 弱化卡片视觉：减少大光球、重阴影和强渐变，改为浅纸卡、轻描边、柔和色晕，整体更接近练习本/情绪地图而不是儿童卡牌。
- 两行布局保持 5 + 4，并让第二行从第 2 列开始，形成更自然的居中错落；小屏仍回退为 3 列。
- 保留长按展开子情绪浮层、点击子情绪填入主情绪的逻辑。
- 已同步更新缓存版本：`index.html` 的 CSS/JS query 参数为 `v=10`，`service-worker.js` 的 `CACHE_NAME` 为 `sedona-workbook-v10`。
